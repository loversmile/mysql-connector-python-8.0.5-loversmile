...
class MysqlRow:
    def __init__(self, tuple_key, tuple_value):
        self.tuple_key = tuple_key
        self.tuple_value = tuple_value

    def __str__(self):
        return str(self.tuple_value)
    
    def __getitem__(self, item):
        if type(item) == type(1):
            return self.tuple_value[item]
        elif type(item) == type('str'):
            return self.tuple_value[self.tuple_key.index(item)]
...
class MySQLCursorMysqlRow(MySQLCursor):
    """
    Cursor fetching rows as dictionaries.
    ...
    def _row_to_python(self, rowdata, desc=None):
        ...
        if row:
            return MysqlRow(self.column_names, row)#dict(zip(self.column_names, row))
